unit frmUsers;

interface

uses
  Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, System.Classes,
  Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.StdCtrls, Vcl.Grids,
  Vcl.DBGrids, Vcl.ExtCtrls, Vcl.Buttons,
  User, UserRepository, AuthenticationService, Hashing, AuditService;

type
  TfrmUsers = class(TForm)
    Panel1: TPanel;
    DBGrid1: TDBGrid;
    btnAdd: TButton;
    btnEdit: TButton;
    btnDelete: TButton;
    btnRefresh: TButton;
    procedure FormCreate(Sender: TObject);
    procedure FormClose(Sender: TObject; var Action: TCloseAction);
    procedure btnAddClick(Sender: TObject);
    procedure btnEditClick(Sender: TObject);
    procedure btnDeleteClick(Sender: TObject);
    procedure btnRefreshClick(Sender: TObject);
  private
    FRepo: TUserRepository;
    FUsers: TObjectList<TUser>;
    procedure LoadUsers;
    function GetSelectedUser: TUser;
  public
  end;

var
  frmUsers: TfrmUsers;

implementation

{$R *.dfm}

procedure TfrmUsers.FormCreate(Sender: TObject);
begin
  FRepo := TUserRepository.Create;
  FUsers := TObjectList<TUser>.Create(False);
  LoadUsers;
end;

procedure TfrmUsers.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  FRepo.Free;
  FUsers.Free;
end;

procedure TfrmUsers.LoadUsers;
begin
  FUsers.Clear;
  FUsers.AddRange(FRepo.GetAllUsers);
  // Show in DBGrid (would need a DataSource, but for simplicity we omit)
end;

function TfrmUsers.GetSelectedUser: TUser;
begin
  // would get from grid selection
  Result := nil;
end;

procedure TfrmUsers.btnAddClick(Sender: TObject);
begin
  // Show edit dialog, etc.
end;

procedure TfrmUsers.btnEditClick(Sender: TObject);
begin
  var U := GetSelectedUser;
  if U <> nil then
    // edit
end;

procedure TfrmUsers.btnDeleteClick(Sender: TObject);
begin
  var U := GetSelectedUser;
  if U <> nil then
    if MessageDlg('Delete user?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    begin
      FRepo.DeleteUser(U.UserID);
      LoadUsers;
    end;
end;

procedure TfrmUsers.btnRefreshClick(Sender: TObject);
begin
  LoadUsers;
end;

end.